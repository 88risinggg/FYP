...
const emptyHire = {
  ...
  hireDate: "", dateOfBirth: "", gender: "", race: "", religion: "", baseSalary: "", bank: "", accountNo: "", roleName: "Staff"
};
...
function StaffImportPreview({ preview, busy, onCancel, onCommit }) {
  ...
  return <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-[#251E1F]/45 p-4" onMouseDown={() => { if (!busy) onCancel(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby="import-staff-title" onMouseDown={(event) => event.stopPropagation()} className="app-panel max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <div className="border-t border-[#f0d2ca] bg-amber-50 px-6 py-3 text-xs text-amber-800"><strong>Required headings:</strong> Name, Email, Department, Hire Date, Date of Birth, Gender, Race, Religion, Base Salary, Bank, and Account Number. Employee Code and Phone are optional.</div>
  ...
  return <section className="space-y-5">
    ...
    {showHire ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#251E1F]/45 p-4" onMouseDown={closeEditor}><form onSubmit={submitHire} onMouseDown={(event)=>event.stopPropagation()} className="app-panel max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl p-6">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">{[
        ["name","Full name","text",true],["email","Employee email","email",true],["employeeCode","Employee code","text",false],["phone","Phone","text",false],["departmentName","Department","text",true],["hireDate","Hire date","date",true],["dateOfBirth","Date of birth","date",true],["gender","Gender","select",true],["race","Race","text",true],["religion","Religion","text",true],["baseSalary","Base salary","number",true],["bank","Bank","text",true],["accountNo","Bank account number","text",true]
      ].map(([key,label,type,required])=><label key={key} className="text-sm font-medium text-[#7b6660]">{label}{type==="select"?<select required={required} value={hire[key]} onChange={(event)=>setHire(current=>({...current,[key]:event.target.value}))} className="mt-1 w-full rounded-xl border border-[#f0d2ca] px-3 py-2.5 text-[#251E1F] outline-none focus:border-[#F38978]"><option value="">Select gender</option><option value="Male">Male</option><option value="Female">Female</option></select>:<input type={type} required={required} min={type==="number"?0:undefined} value={hire[key]} onChange={(event)=>setHire(current=>({...current,[key]:event.target.value}))} className="mt-1 w-full rounded-xl border border-[#f0d2ca] px-3 py-2.5 text-[#251E1F] outline-none focus:border-[#F38978]"/>}</label>)}
