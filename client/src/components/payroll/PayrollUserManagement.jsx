  ...
  return <section className="space-y-5">
    ...
    {showHire ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#251E1F]/45 p-4" onMouseDown={closeEditor}><form onSubmit={submitHire} onMouseDown={(event)=>event.stopPropagation()} className="app-panel max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl p-6">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">{[
        ...
        <div key="gender"><label className="block text-xs font-medium text-[#7b6660] mb-1">Gender <span className="text-red-500">*</span></label><select value={hire.gender || ""} onChange={(e) => setHire(h => ({ ...h, gender: e.target.value }))} className="w-full rounded-xl border border-[#f0d2ca] px-3 py-2.5 text-sm text-[#251E1F] outline-none focus:border-[#F38978]" required><option value="">Select gender</option><option value="Male">Male</option><option value="Female">Female</option></select></div>
