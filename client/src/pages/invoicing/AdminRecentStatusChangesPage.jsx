import { AlertCircle, ArrowLeft, ArrowRight, ChevronLeft, ChevronRight, Loader2, RefreshCw, Search } from "lucide-react";
...
const invoiceListPath = "/dashboard/invoicing/admin/invoices";
const ranges = [
...
export default function AdminRecentStatusChangesPage() {
  ...
  return (
    <main className="min-h-screen bg-[#fff8f5] px-4 py-6 text-[#251E1F] sm:px-8 lg:px-12">
      <div className="mx-auto max-w-[1500px] space-y-5">
        <Link to={performancePath} className="inline-flex items-center gap-2 rounded-lg border border-[#f0d2ca] bg-white px-4 py-2 text-sm font-bold text-[#6f4f47] hover:border-[#F38978] hover:text-[#F38978] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F38978]"><ArrowLeft size={17} /> Back to Invoice Performance</Link>
        <header><h1 className="text-3xl font-bold">Recent Status Changes</h1><p className="mt-1 text-sm text-[#7b6660]">Review real invoice status-history records from the shared invoicing data.</p></header>
        ...
        <section className="rounded-2xl border border-[#f0d2ca] bg-white p-5 shadow-[0_10px_28px_rgba(37,30,31,0.06)]">
          <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
            ...
            <form onSubmit={submitSearch} className="flex gap-2"><label className="relative"><Search size={16} className="absolute left-3 top-3 text-[#9c7b72]" /><span className="sr-only">Search invoices or customers</span><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Invoice # or customer" className="w-full rounded-lg border border-[#f0d2ca] py-2 pl-9 pr-3 text-sm outline-none focus:border-[#F38978] sm:w-64" /></label><button type="submit" className="rounded-lg bg-[#F38978] px-4 py-2 text-sm font-bold text-white hover:bg-[#df7464]">Search</button></form>
          </div>

          {validationError ? <p className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">{validationError}</p> : error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"><p className="flex items-center gap-2"><AlertCircle size={17} />{error}</p><button type="button" onClick={load} className="mt-3 inline-flex items-center gap-2 font-bold"><RefreshCw size={15} /> Retry</button></div> : loading && !data ? <div className="flex min-h-72 items-center justify-center gap-2 text-[#7b6660]"><Loader2 size={20} className="animate-spin" /> Loading status changes...</div> : changes.length ? <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead><tr className="border-y border-[#f0d2ca] bg-[#fff8f5] text-xs uppercase text-[#7b6660]"><th className="px-4 py-3">Invoice #</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Status Change</th><th className="px-4 py-3">Changed On</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3">Changed By</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody>{changes.map((change) => <tr key={change.id} className="border-b border-[#f5e8e3]"><td className="px-4 py-3">{change.invoiceId ? <Link to={`${invoiceListPath}?invoiceId=${encodeURIComponent(change.invoiceId)}`} className="font-bold text-[#F38978] hover:underline">{change.invoiceNo || "-"}</Link> : <span className="font-bold">{change.invoiceNo || "-"}</span>}</td><td className="px-4 py-3">{change.customerName || "-"}</td><td className="px-4 py-3"><span className="inline-flex items-center gap-2">{change.fromStatus}<ArrowRight size={14} className="text-[#F38978]" /><strong>{change.toStatus}</strong></span></td><td className="whitespace-nowrap px-4 py-3 text-[#7b6660]">{formatDate(change.changedOn)}</td><td className="whitespace-nowrap px-4 py-3 text-right font-semibold">{formatCurrency(change.amount)}</td><td className="px-4 py-3">{change.changedBy || "System"}</td><td className="px-4 py-3 text-right">{change.invoiceId ? <Link to={`${invoiceListPath}?invoiceId=${encodeURIComponent(change.invoiceId)}`} className="font-bold text-[#F38978] hover:underline">View Invoice</Link> : <span className="text-[#9c7b72]">Unavailable</span>}</td></tr>)}</tbody></table></div> : <p className="rounded-xl border border-dashed border-[#f0c9bf] bg-[#fff8f5] p-12 text-center text-sm text-[#7b6660]">No status changes found for these filters.</p>}
