    ...
    >
      {upload ? (
        <section className="rounded-lg border border-[#f0d2ca] bg-white p-5">
          ...
          {data?.categorySummary?.length ? <div className="mt-4 flex flex-wrap gap-2 border-t border-[#f4ded7] pt-4" aria-label="Error category summary">{data.categorySummary.map((item) => <span key={item.category} className="rounded-full bg-[#fff0eb] px-3 py-1 text-xs font-bold text-rose-700">{item.category}: {item.count}</span>)}</div> : null}
        </section>
      ) : null}
