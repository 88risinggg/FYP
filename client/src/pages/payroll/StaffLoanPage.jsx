    ...
    <>
      ...
      <Panel title="Request a Loan">
        <div className="space-y-4">
          {/* Amount */}
          <div>
            <label className="block text-xs text-[#d8c6e8]">Loan Amount ($)</label>
            <input
              type="number"
              min="1"
              max="50000"
          ...
          <div>
            <label className="block text-xs text-[#d8c6e8]">Repayment Period (months)</label>
            <select
              ...
              className={`mt-1 w-full rounded-md border px-3 py-2 text-white bg-transparent ${
                errors.repaymentMonths ? "border-red-400/60" : "border-white/10"
              } ${!repaymentMonths ? "text-white/20" : ""}`}
              style={{ backgroundColor: "#12071f" }}
            >
              <option value="" disabled>Select repayment period</option>
              {Array.from({ length: 36 }, (_, i) => i + 1).map((m) => (
                <option key={m} value={m} style={{ backgroundColor: "#12071f", color: "#fff" }}>
                  {m} {m === 1 ? "month" : "months"}
                </option>
              ))}
            </select>
    ...
    <Panel title="Your Loan Requests">
      {loans.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="mt-4 text-sm text-[#d8c6e8]">No loan requests yet</p>
          <p className="mt-1 text-xs text-white/30">Submit a loan request above to get started.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {loans.map((loan) => {
            ...
            return (
              <div key={loanId} className="rounded-xl border border-white/10 bg-black/10 p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-3">
                      <p className="text-sm font-semibold text-white">{formatCurrency(loan.requested_amount)}</p>
                    ...
                    <p className="mt-1 text-xs text-[#d8c6e8] line-clamp-1">{loan.reason || "—"}</p>
                  ...
                  <div className="flex items-center gap-3">
                    <p className="text-xs text-white/30 whitespace-nowrap">{formatDate(loan.created_at)}</p>
                    {isApproved && (
                      <button
                        type="button"
                ...
                {loan.hr_comments && (
                  <div className="mt-2 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2">
                    <p className="text-xs text-[#d8c6e8]/70"><span className="font-medium text-[#d8c6e8]">HR:</span> {loan.hr_comments}</p>
                  </div>
                )}
                ...
                {isApproved && isExpanded && (
                  <div className="mt-3 rounded-lg border border-white/5 bg-black/20 p-3">
                    {loadingInstallments[loanId] ? (
                      ...
                      <p className="text-xs text-white/40 text-center py-3">No installments found.</p>
                    ) : (
                        ...
                        <table className="w-full text-xs">
                          ...
                          <tbody>
                            {installments[loanId].map((inst) => (
                              <tr key={inst.installment_id} className="border-b border-white/5 last:border-b-0">
                                <td className="py-2 pr-4 text-white/60">{inst.installment_number}</td>
                                <td className="py-2 pr-4 text-white">{formatCurrency(inst.amount)}</td>
                                <td className="py-2 pr-4 text-white/60">{formatDate(inst.due_date)}</td>
                                <td className="py-2">
